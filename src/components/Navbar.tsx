    ...
    >
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex items-center gap-1">
            ...
            {["Case Studies", "Resources", "Who We Are", "Contact"].map((link) => (
              <a
                key={link}
      ...
      {mobileOpen && (
        <div className="md:hidden bg-card border-t border-border px-6 py-4 flex flex-col gap-2">
          {["Home", "Case Studies", "Resources", "Who We Are", "Contact"].map((link) => (
            <a
              key={link}
              href={`#${link.toLowerCase().replace(/\s+/g, "-")}`}
          ...
          {["Demand Planning EER", "Advanced Forecast Diagnostics", "Forecast Health Review"].map((s) => (
            <a
              key={s}
              href="#solutions"
